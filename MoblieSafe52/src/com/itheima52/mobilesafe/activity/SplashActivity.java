...
public class SplashActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences preferences = getSharedPreferences("config",
				MODE_PRIVATE);
		boolean mPre = preferences.getBoolean("auto_update", true);
		...
		// 设置渐变动画
		AlphaAnimation alphaAnimation = new AlphaAnimation(0f, 1.f);
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					URL url = new URL("http://192.168.135.2:8080/update.json");
	...
	private void copyDB() {
		...
		if (file.exists()) {
			...
			try {
				...
				InputStream in = assets.open("address.db");
